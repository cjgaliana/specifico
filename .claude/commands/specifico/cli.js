...
function findSpecDirById(projectRoot2, id) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith(`${id}-`)) {
      return import_path.default.join(specifico, entry.name);
    }
  }
  return null;
}
...
// src/types.ts
var Phase = external_exports.enum([
  "spec",
  "plan",
  "tasks",
  "execute",
...
var StateSchema = external_exports.object({
  ...
  completedTasks: external_exports.array(external_exports.string()),
  blockers: external_exports.array(external_exports.string()),
  verifyPassed: external_exports.boolean().optional()
});
...
var EntitySchema = external_exports.object({
  ...
  fields: external_exports.record(external_exports.string(), EntityFieldSchema)
});
var ApiEndpointSchema = external_exports.object({
  path: external_exports.string(),
  method: external_exports.string(),
  description: external_exports.string().optional(),
  requestSchema: external_exports.record(external_exports.unknown()).optional(),
  responseSchema: external_exports.record(external_exports.unknown()).optional()
});
var AcceptanceCriteriaSchema = external_exports.array(external_exports.string());
var SpecSchema = external_exports.object({
  ...
  entities: external_exports.array(EntitySchema),
  apiEndpoints: external_exports.array(ApiEndpointSchema),
  ...
  createdAt: external_exports.string(),
  updatedAt: external_exports.string(),
  verifyResults: external_exports.array(
    external_exports.object({
      criterion: external_exports.string(),
      verdict: external_exports.enum(["pass", "fail", "partial"]),
...
var TaskSchema = external_exports.object({
  ...
  status: TaskStatus,
  completedSha: external_exports.string().optional()
});
var TasksSchema = external_exports.object({
  specId: external_exports.string(),
...
var MemoryDeltaSchema = external_exports.object({
  entities: external_exports.array(EntitySchema),
  apiEndpoints: external_exports.array(ApiEndpointSchema)
});
var ConflictSchema = external_exports.object({
  severity: external_exports.enum(["block", "warn"]),
  field: external_exports.string(),
  message: external_exports.string()
});
var JournalSpecSchema = external_exports.object({
  id: external_exports.string(),
  slug: external_exports.string(),
  title: external_exports.string(),
...
var JournalSchema = external_exports.object({
  version: external_exports.number(),
  specs: external_exports.array(JournalSpecSchema)
});
var CliResultSchema = external_exports.discriminatedUnion("success", [
...
function readState(specDir) {
  return StateSchema.parse(readJson(import_path2.default.join(specDir, STATE_FILE)));
}
function writeState(specDir, state) {
  StateSchema.parse(state);
  writeJson(import_path2.default.join(specDir, STATE_FILE), state);
}
...
function transition(specDir, toPhase) {
...
function initJournal(projectRoot2) {
  ...
  const jPath = journalPath(projectRoot2);
  if (!import_fs2.default.existsSync(jPath)) {
    const journal = { version: 1, specs: [] };
    writeJournal(projectRoot2, journal);
    return journal;
  }
  return readJournal(projectRoot2);
}
function readJournal(projectRoot2) {
  ensureSpecificoDir(projectRoot2);
  const jPath = journalPath(projectRoot2);
  if (!import_fs2.default.existsSync(jPath)) {
  ...
  try {
    const content = import_fs2.default.readFileSync(jPath, "utf-8");
    return JSON.parse(content);
  } catch (err) {
    throw new Error(`Failed to read journal.json: ${err}`);
  }
}
function writeJournal(projectRoot2, journal) {
  ensureSpecificoDir(projectRoot2);
  const jPath = journalPath(projectRoot2);
  const tmpPath = jPath + ".tmp";
  import_fs2.default.writeFileSync(tmpPath, JSON.stringify(journal, null, 2), "utf-8");
  import_fs2.default.renameSync(tmpPath, jPath);
}
function addSpecToJournal(projectRoot2, id, slug, title, phase, branch, createdAt) {
  const journal = readJournal(projectRoot2);
...
function updateSpecInJournal(projectRoot2, id, phase, updatedAt) {
  const journal = readJournal(projectRoot2);
...
function getNextId(projectRoot2) {
  const journal = initJournal(projectRoot2);
...
function rebuildJournalFromDirectory(projectRoot2) {
  ensureSpecificoDir(projectRoot2);
  const specDirs = listAllSpecDirs(projectRoot2);
  const specs = [];
  for (const specDir of specDirs) {
    try {
      ...
      specs.push({
        ...
        slug: state.slug,
        title: state.id,
        // Fallback: we'll try to get title from STATE.json if available
        phase: state.phase,
        branch: state.branch,
  ...
  specs.sort((a, b2) => parseInt(a.id, 10) - parseInt(b2.id, 10));
  const journal = { version: 1, specs };
  writeJournal(projectRoot2, journal);
  return journal;
}
...
// src/id.ts
function nextId(projectRoot2) {
  return getNextId(projectRoot2);
}
...
var init_parse_commit = __esm({
  "src/lib/parsers/parse-commit.ts"() {
    ...
    init_utils();
    parsers = [
      new LineParser(/^\[([^\s]+)( \([^)]+\))? ([^\]]+)/, (result, [branch, root, commit2]) => {
...
var init_BranchSummary = __esm({
  "src/lib/responses/BranchSummary.ts"() {
    ...
    BranchSummaryResult = class {
      ...
      push(status, detached, name, commit2, label) {
        if (status === "*") {
  ...
  "src/lib/parsers/parse-branch.ts"() {
    ...
    parsers9 = [
      new LineParser(
        ...
        (result, [current, name, commit2, label]) => {
...
function deleteBranchTask(branch, forceDelete = false) {
  const task = {
    ...
    onError({ exitCode, stdErr, stdOut }, error, _2, fail2) {
      if (!hasBranchDeletionError(String(error), exitCode)) {
        return fail2(error);
      }
...
var require_git = __commonJS2({
  "src/git.js"(exports2, module2) {
    ...
    Git2.prototype.revert = function(commit2) {
      ...
      return this._runTask(
        straightThroughStringTask2(["revert", ...getTrailingOptions2(arguments, 0, true), commit2]),
        next
      );
    };
...
// src/git.ts
var COMMIT_RE = /^specifico\(\d{3}\/(T\d{3}|fix)\): .+/;
function git(projectRoot2) {
  return esm_default(projectRoot2);
}
async function branchCreate(projectRoot2, branch) {
  await git(projectRoot2).checkoutLocalBranch(branch);
}
async function commit(projectRoot2, message) {
  if (!COMMIT_RE.test(message)) {
    throw new Error(
      `Commit message must match: specifico(<id>/T<nnn>|fix): <title>. Got: "${message}"`
    );
  }
  await git(projectRoot2).add(".");
  const result = await git(projectRoot2).commit(message);
  return result.commit;
}
async function mergeBranch(projectRoot2, branch) {
  await git(projectRoot2).merge(["--no-ff", branch]);
}
async function currentBranch(projectRoot2) {
  const result = await git(projectRoot2).revparse(["--abbrev-ref", "HEAD"]);
  return result.trim();
}
async function branchExists(projectRoot2, branch) {
  const result = await git(projectRoot2).branch(["-a"]);
  return Object.keys(result.branches).some(
    (b2) => b2 === branch || b2 === `remotes/origin/${branch}`
  );
}
async function getSha(projectRoot2, ref) {
  const result = await git(projectRoot2).revparse([ref]);
  return result.trim();
}
...
function readSpec(specDir) {
  return SpecSchema.parse(readJson(import_path4.default.join(specDir, SPEC_FILE)));
}
...
function rebuildMemory(projectRoot2) {
...
function readTasks(specDir) {
  return TasksSchema.parse(readJson(import_path6.default.join(specDir, TASKS_FILE)));
}
function writeTasks(specDir, tasks) {
  TasksSchema.parse(tasks);
  writeJson(import_path6.default.join(specDir, TASKS_FILE), tasks);
}
function nextReadyTask(tasks) {
  const doneIds = new Set(
    tasks.tasks.filter((t2) => t2.status === "done").map((t2) => t2.id)
  );
  return tasks.tasks.find(
    (t2) => t2.status === "pending" && t2.dependsOn.every((dep) => doneIds.has(dep))
  ) ?? null;
}
function markTaskDone(specDir, taskId, sha) {
  const tasks = readTasks(specDir);
  const task = tasks.tasks.find((t2) => t2.id === taskId);
...
function validateNoCycles(tasks) {
  ...
  function dfs(id) {
    ...
    inStack.add(id);
    visited.add(id);
    const task = tasks.tasks.find((t2) => t2.id === id);
    if (task) {
  ...
  try {
    ...
    if (cmd === "git") {
      ...
      if (sub === "branch-create") {
        await branchCreate(projectRoot, args[1]);
        return ok({ branch: args[1] });
      }
      if (sub === "commit") {
        const sha = await commit(projectRoot, args.slice(1).join(" "));
        return ok({ sha });
      }
      if (sub === "merge") {
        await mergeBranch(projectRoot, args[1]);
        return ok({ merged: args[1] });
      }
      if (sub === "current-branch") {
        return ok(await currentBranch(projectRoot));
      }
      if (sub === "branch-exists") {
        return ok(await branchExists(projectRoot, args[1]));
      }
      if (sub === "get-sha") {
        return ok(await getSha(projectRoot, args[1] ?? "HEAD"));
      }
