...
function nextId(projectRoot2) {
  ...
  const counterPath = import_path2.default.join(projectRoot2, "specifico", ".counter");
  const tmpPath = counterPath + ".tmp";
  let current = 0;
  if (import_fs2.default.existsSync(counterPath)) {
    current = parseInt(import_fs2.default.readFileSync(counterPath, "utf-8").trim(), 10);
  }
  const next = current + 1;
  import_fs2.default.writeFileSync(tmpPath, String(next), "utf-8");
  import_fs2.default.renameSync(tmpPath, counterPath);
...
function readState(specDir) {
  return StateSchema.parse(readJson(import_path3.default.join(specDir, STATE_FILE)));
}
function writeState(specDir, state) {
  StateSchema.parse(state);
  writeJson(import_path3.default.join(specDir, STATE_FILE), state);
}
